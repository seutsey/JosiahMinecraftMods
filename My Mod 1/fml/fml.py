...
def pre_decompile(mcp_dir, fml_dir, disable_assets=False):
    copy_minecraft(mcp_dir, fml_dir)
...
def copy_mcp(fml_dir, mcp_dir, version=None):
    mc_info = read_mc_versions(fml_dir, version=version)
    
    yd_dir = os.path.join(fml_dir, '..', 'yd')
    ver = mc_info['mcp_ver'].replace(".", "")    
    yd_mcp_file = os.path.join(yd_dir, 'downloaded', 'mcp'+ver+'.zip') #custom
    
    print('Copying MCP zip from yds local')
    
    if (os.path.exists(yd_mcp_file) and not os.path.exists(mc_info['mcp_file'])):
        shutil.copyfile(yd_mcp_file,mc_info['mcp_file'])
...
#==========================================================================
#                      Copy Functions!
# This relies on existance of .minecraft/jars directory which is a copy of
# mcp/jars directory downloaded by download_minecraft function
# mcp/jars directory is deleted first
#==========================================================================    
def copy_minecraft(mcp_dir, fml_dir, version=None):
    failed = False
    ...
    yd_natives_dir = os.path.join(yd_dir, 'included', mc_info['version']+'-natives') #custom
    ...
    print 'Copying minecraft libraries and files'
    try:
        #copy all of forgelocal libraries directory
        print 'Copying %s to %s' % (yd_libraries_dir, mc_info['library_dir'])
        shutil.rmtree(mc_info['library_dir']) 
        shutil.copytree(yd_libraries_dir, mc_info['library_dir'])

        #copy fml json file    
        print 'Copying %s to %s' % (fml_json_file, mc_info['json_file'])           
        shutil.copyfile(fml_json_file,mc_info['json_file'])     
        
        #copy or download client jar        
        if os.path.exists(minecraft_client_file):
            print 'Copying %s to %s' % (minecraft_client_file, mc_info['client_file'])
            shutil.copyfile(minecraft_client_file,mc_info['client_file'])            
        else:
            print '%s not found. Downloading from %s' % (minecraft_client_file, mc_info['client_url'])
            failed = not download_file(mc_info['client_url'], mc_info['client_file'], None) or failed
        file_backup(mc_info['client_file'], None)        
        
        #copy or download server jar
        if os.path.exists(minecraft_server_file):
            print 'Copying %s to %s' % (minecraft_server_file, mc_info['server_file'])
            shutil.copyfile(minecraft_server_file,mc_info['server_file'])         
        else:
            print '%s not found. Downloading from %s' % (minecraft_server_file, mc_info['server_url'])
            failed = not download_file(mc_info['server_url'], mc_info['server_file'], None) or failed
        file_backup(mc_info['server_file'], None)
        
        #copy natives directory
        print 'Copying %s to %s' % (yd_natives_dir, mc_info['natives_dir'])
        shutil.rmtree(mc_info['natives_dir']) 
        shutil.copytree(yd_natives_dir, mc_info['natives_dir'])
        
        if failed:
            print 'Failed copying minecraft libraries and files'
            sys.exit(1)
